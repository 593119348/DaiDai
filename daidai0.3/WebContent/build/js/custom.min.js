...
function init_cropper() {
    if ("undefined" != typeof $.fn.cropper) {
        ...
        l ? k.change(function() {
            ...
            function() {
                l.revokeObjectURL(m)
            }).cropper("reset").cropper("replace", m)) : window.alert("Please choose an image file."))
...
function init_echarts() {
    if ("undefined" != typeof echarts) {
        ...
        if ($("#echart_line").length) {
            ...
            $.post("/daidai/loginrecodecount",{},function(data){
                var strarr = data.split(",");
                
                f.setOption({
                    title: {
                        text: "Line Graph",
                        subtext: "Log in this week"
                    },
                    tooltip: {
                        trigger: "axis"
                    },
        ...
        if ($("#echart_bar_horizontal").length) {
            ...
            $.post("/daidai/toploginrecodecount",{},function(data){
            	var mydata = data.substr(1,data.length-2).replace(/\ /g, "").replace(/\{/g, "").replace(/\}/g, "").replace(/\user_phone=/g, "").replace(/count=/g, "").split(",");
            	
                b.setOption({
                	title: {
                		text: "Top Login Times For User",
                		subtext: "Log in this week"
                	},
                	tooltip: {
                		trigger: "axis"
                	...
                	yAxis: [{
                		type: "category",
                		data: [mydata[9], mydata[7], mydata[5], mydata[3], mydata[1]]
                	}],
                	series: [{
                		name: "Login Times ",
                		type: "bar",
                		data: [mydata[8], mydata[6], mydata[4], mydata[2], mydata[0]]
                	}]
                })
            });  
        }
